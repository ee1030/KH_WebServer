...
public class EncryptWrapper extends HttpServletRequestWrapper {
	...
	public String getParameter(String name) {
		...
		case "currentPwd" : // 비밀번호 변경 시 현재 비밀번호 name 속성값
		case "newPwd1" : // 비밀번호 변경 시 새 비밀번호 name 속성값
	...
	public static String getSha512(String pwd) {
		...
		// MessageDigest : 지정한 알고리즘을 따라 해시함수를 진행하는 객체
		// 해시함수 : 입력 값을 여러 단계의 연산을 거쳐 일정한 길이의 무작위 값을 만들어내는 함수
		
		try {
			md = MessageDigest.getInstance("SHA-512");
			// 지정된 알고리즘(SHA-512)을 이용할 수 있는 MessageDigest 객체를 생성 후 얻어옴.
